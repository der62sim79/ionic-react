...
const App: React.FC = () => {
  ...
  const calculateBMI = () => {
    ...
    if (
      (calcUnits === "mkg" && bmi <= 18.5) ||
      (calcUnits === "ftlbs" && bmi <= 21)
    ) {
      setColor("danger");
      setUserInfo("Du bist untergewichtig!");
    } else if (
      (calcUnits === "mkg" && bmi >= 18.51 && bmi <= 25) ||
      (calcUnits === "ftlbs" && bmi >= 21.01 && bmi <= 26)
    ) {
      setColor("primary");
      setUserInfo("Du hast dein ideal Gewicht!");
    } else if (
      (calcUnits === "mkg" && bmi >= 25.1 && bmi <= 30) ||
      (calcUnits === "ftlbs" && bmi >= 26.1 && bmi <= 31)
    ) {
      setColor("warning");
      setUserInfo("Du hast leichten übergewicht!");
    } else if (
      (calcUnits === "mkg" && bmi >= 30.1) ||
      (calcUnits === "ftlbs" && bmi >= 30.1)
    ) {
      setColor("danger");
          ...
          <IonGrid>
                    ...
                    <IonGrid className="ion-no-padding">
                        ...
                        <IonCol>
                          <IonItem>
                            <IonLabel position="floating">
                              {calcUnits === "mkg"
                                ? "Gib deine Körpergröße in Meter ein"
                                : "Gib deine Körpergröße in feet ein"}
                            </IonLabel>
                            <IonInput
                              type="number"
                              ref={heightInputRef}
                      ...
                      <IonRow>
                        <IonCol>
                          <IonItem>
                            <IonLabel position="floating">
                              {calcUnits === "mkg" ? "Gewicht in kg": "Gewicht in lbs"}
                            </IonLabel>
                            <IonInput
                              type="number"
                              ref={weightInputRef}
            ...
            <IonRow>
              ...
              {calculatedBmi && (
                        <BmiResult
                          color={color}
                          calcResultUser={+calculatedBmi}
                          bmiResultUserInfo={userInfo}
                        />
                      )}
              </IonCol>
            </IonRow>
